...
public class MySolidSpawner : MonoBehaviour
{
    ...
    public void BossSpawn()
    {
        ...
        float angle = Random.Range(0f, 360f);
        float x = movableObject.transform.position.x + desiredCircleRadius* Random.Range(2, 4) * Mathf.Cos(angle);
        screen.ShowBossText();
        float z = movableObject.transform.position.z + desiredCircleRadius *Random.Range(2, 4) * Mathf.Sin(angle);
        Vector3 position = new(x, 0, z);
        prefab.transform.SetPositionAndRotation(position, Quaternion.identity);
        Instantiate(prefab);
    }
